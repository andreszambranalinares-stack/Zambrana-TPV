...
export function renderMobileCamarero(container, app) {
    ...
    const showGuestModal = (onConfirm) => {
        ...
        ov.innerHTML = `
            <div style="background:var(--color-surface);border:1px solid var(--color-border);border-radius:20px;padding:1.75rem;width:min(340px,92vw);text-align:center;">
                <div style="font-size:1.3rem;font-weight:800;margin-bottom:.2rem;">👥 Comensales</div>
    ...
    const showPayModal = () => {
        ...
        ov.innerHTML = `
            <div style="background:var(--color-surface);border:1px solid var(--color-border);border-radius:20px;padding:1.75rem;width:min(340px,92vw);text-align:center;">
                <div style="font-size:1.2rem;font-weight:800;margin-bottom:.2rem;">💳 Cobrar Mesa ${String(activeTableId).padStart(2,'0')}</div>
                ...
                <button class="pay-btn" data-m="${m}" style="display:flex;align-items:center;gap:.75rem;width:100%;padding:.9rem 1rem;border-radius:12px;border:2px solid var(--color-border);background:var(--color-bg);color:var(--color-text);font-size:1rem;font-weight:700;cursor:pointer;margin-bottom:.5rem;">
                    <span>${m==='efectivo'?'💵':m==='tarjeta'?'💳':'🤝'}</span>${m.charAt(0).toUpperCase()+m.slice(1)}
                </button>`).join('')}
        ...
        ov.querySelectorAll('.pay-btn').forEach(btn => {
            ...
            btn.addEventListener('click', () => {
                const method = btn.dataset.m.toUpperCase();
                tickets.addTicket({ type:'cobro', tableId:activeTableId, waiter:app.currentUser?.alias||'Camarero', total, method, htmlContent:`<div>Mesa ${activeTableId} | ${method} | ${total.toFixed(2)}€</div>` });
                ...
                ov.remove();
                app.showToast(`✅ Mesa ${activeTableId} cobrada — ${total.toFixed(2)}€`);
    ...
    const buildMesas = () => {
        ...
        <div class="mob-full">
            <div class="mob-header">
                ...
                <div style="background:rgba(255,255,255,.15);border-radius:20px;padding:.25rem .75rem;color:#fff;font-size:.8rem;font-weight:700;">${new Date().toLocaleTimeString('es-ES',{hour:'2-digit',minute:'2-digit'})}</div>
            </div>
            <div class="mob-scroll" style="padding:1rem;">
                <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(88px,1fr));gap:.75rem;">
                    ${tables.map(t => {
                        ...
                        const total = orders.reduce((s,o)=>s+o.items.reduce((ss,i)=>ss+i.price*i.qty,0),0);
                        return `
                        <div class="mob-table-tile" data-table-id="${t.id}" style="background:${occ?'var(--color-primary)':'var(--color-surface)'};border:2px solid ${occ?'var(--color-primary-light)':'var(--color-border)'};border-radius:14px;padding:.85rem .5rem;text-align:center;cursor:pointer;transition:transform .12s;position:relative;">
                            <div style="font-size:1.3rem;font-weight:800;color:${occ?'#fff':'var(--color-text)'};">${String(t.id).padStart(2,'0')}</div>
                            ${t.guests?`<div style="font-size:.7rem;color:${occ?'rgba(255,255,255,.75)':'var(--color-text-muted)'};">${t.guests} <i class='bx bx-user'></i></div>`:''}
                            ${total?`<div style="font-size:.78rem;font-weight:700;color:${occ?'rgba(255,255,255,.9)':'var(--color-primary-light)'};">${total.toFixed(2)}€</div>`:''}
                            <div style="font-size:.62rem;margin-top:.2rem;color:${occ?'rgba(255,255,255,.6)':'var(--color-text-muted)'};">${occ?'Ocupada':'Libre'}</div>
                            ${occ?`<div style="position:absolute;top:-6px;right:-6px;width:12px;height:12px;background:#22c55e;border-radius:50%;border:2px solid var(--color-bg);"></div>`:``}
                        </div>`;
                    }).join('')}
                </div>
        ...
        <div id="admin-drawer" class="${drawerOpen?'':'hidden'}">
            ...
            <div id="admin-drawer-panel">
                <div style="padding:1.25rem 1.25rem 1rem;border-bottom:1px solid var(--color-border);display:flex;align-items:center;gap:.75rem;">
                    <div style="width:42px;height:42px;border-radius:50%;background:var(--color-primary);display:flex;align-items:center;justify-content:center;color:#fff;font-weight:800;font-size:1.1rem;">${(app.currentUser?.alias||'?').charAt(0)}</div>
                    <div>
        ...
        <div class="mob-full">
            <div class="mob-header">
                ...
                <div style="margin-left:.5rem;flex:1;">
                    ...
                    <div class="mob-header-sub">${t.guests||'?'} comensales${total?' · '+total.toFixed(2)+'€':''}</div>
                </div>
            </div>
            <div class="mob-scroll" style="padding:1rem;display:flex;flex-direction:column;gap:.75rem;">
                ...
                <div style="background:var(--color-surface);border-radius:14px;padding:1rem;border:1px solid var(--color-border);">
                    <div style="font-size:.68rem;font-weight:800;text-transform:uppercase;letter-spacing:.07em;color:var(--color-text-muted);margin-bottom:.5rem;">Enviado a cocina</div>
    ...
    const buildComanda = () => {
        ...
        // Category icons
        const catIcons = { 'Carne':'🥩','Pescado':'🐟','Entrante':'🥗','Postre':'🍮','Postres':'🍮','Bebidas':'🍺','Bebida':'🍺','Pizza':'🍕','Pasta':'🍝','Ensalada':'🥗' };
        const icon = c => catIcons[c] || '🍽️';
            ...
            <div class="pending-strip">
                ...
                <div class="pending-row">
                    ...
                    <div style="display:flex;align-items:center;gap:.6rem;">
                        ...
                        <button data-remove="${idx}" style="background:none;border:none;color:var(--color-primary-light);cursor:pointer;font-size:1rem;padding:0 2px;min-height:auto;">✕</button>
                    </div>
                </div>`).join('')}
            </div>`:''}
